        ...
        <div className="product-cards group rounded overflow-hidden border border-gray-200">
            <Link
                ...
                <div className="h-[185px] w-full relative">
                    <Image sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" fill blurDataURL="./product_placeholder.png" src={images.length == 0 ? '/product_placeholder.png' : images[0].imageUrl} alt="image" />
                </div>
                <div className="prduct_name mb-2 line-clamp-2">{name}</div>
                <div className="prix">{priceFormatter.format(price)}</div>
            ...
            {/* <button
                ...
                className="opacity-0 group-hover:opacity-100 w-[90%] block bg-orange-500 text-center text-white text-md uppercase mt-5 py-[8px] rounded-sm m-auto"
            >
                Ajouter au panier
            </button> */}
            <div className="absolute top-1 right-1 bg-red-500/20 text-red-500 rounded p-1">-50%</div>
        </div>
    );
};
